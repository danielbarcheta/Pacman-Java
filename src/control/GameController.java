...
public class GameController {
    ...
    public void processAllElements(ArrayList<Element> elements, int [][]matrix, int cont){
        ...
        if(overlapGhostPacman) { 
        	...
        	if(Main.level>=4){
        		Main.gamePacMan.dispose();
        		JOptionPane.showMessageDialog(null, "Fim do jogo, parabéns!");
        		System.exit(0);
        	}
        	else{
        		Main.gamePacMan.reStartGame(pacman.getLifes());  // era reStartGame(1), alterei!
        	}
        }
        else{
	...
	private void checkPacmanEatSomeOneAndOrTimeFruittoDesappear(ArrayList<Element> elements, Pacman pacman) {
        ...
        for(int i =1; i < elements.size(); i++){
            ...
            if(pacman.overlap(eTemp)){
                ...
                if(remainingScore>10000){          // valor alterado de 1000 para 10000, para que assim não se ganhe vida
                	pacman.addLife();              // comendo-se fantasmas! bn
        	...
        	if(elapsedTimePower>=7000){
        		...
        		for (int i=1;i<=pacman.getNumberGhosttoEat();i++){
        			...
        			if (e instanceof Blank) {
        				((Blank) e).changeGhosttoNormal("blank.png");
        			}
